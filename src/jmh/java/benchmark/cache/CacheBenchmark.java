...
public class CacheBenchmark {
    ...
    @Benchmark
    public boolean list_search_fastList_contains() {
        return fastList.contains(size / 2);
    }

    @Benchmark
    public boolean list_search_immutableList_contains() {
        return immutableList.contains(size / 2);
    }

    @Benchmark
    public boolean list_search_intArrayList_contains() {
        return intArrayList.contains(size / 2);
    }

    @Benchmark
    public int list_search_fastList_detect() {
        Integer value = fastList.detect(each -> each == size / 2);
        return value == null ? -1 : value;
    }

    @Benchmark
    public int list_search_immutableList_detect() {
        Integer value = immutableList.detect(each -> each == size / 2);
        return value == null ? -1 : value;
    }

    @Benchmark
    public boolean list_search_intArrayList_anySatisfy() {
        return intArrayList.anySatisfy(each -> each == size / 2);
    }

    @Benchmark
    public boolean map_search_unifiedMap_containsKey() {
        return unifiedMap.containsKey(size / 2);
    }

    @Benchmark
    public boolean map_search_immutableMap_containsKey() {
        return immutableMap.containsKey(size / 2);
    }

    @Benchmark
    public boolean map_search_intIntHashMap_containsKey() {
        return intIntMap.containsKey(size / 2);
    }

    @Benchmark
    public int map_search_unifiedMap_get() {
        Integer value = unifiedMap.get(size / 2);
        return value == null ? -1 : value;
    }

    @Benchmark
    public int map_search_immutableMap_get() {
        Integer value = immutableMap.get(size / 2);
        return value == null ? -1 : value;
    }

    @Benchmark
    public int map_search_unifiedMap_getIfAbsent() {
        return unifiedMap.getIfAbsent(size / 2, () -> -1);
    }

    @Benchmark
    public int map_search_intIntHashMap_get() {
        return intIntMap.get(size / 2);
    }

    @Benchmark
    public int map_search_intObjectHashMap_get() {
        Integer value = intObjectMap.get(size / 2);
        return value == null ? -1 : value;
    }

    @Benchmark
    public boolean set_search_unifiedSet_contains() {
        return unifiedSet.contains(size / 2);
    }

    @Benchmark
    public boolean set_search_immutableSet_contains() {
        return immutableSet.contains(size / 2);
    }

    @Benchmark
    public boolean set_search_intHashSet_contains() {
        return intHashSet.contains(size / 2);
    }

    @Benchmark
    public boolean set_search_unifiedSet_anySatisfy() {
        return unifiedSet.anySatisfy(value -> value == size / 2);
    }

    @Benchmark
    public boolean set_search_intHashSet_anySatisfy() {
        return intHashSet.anySatisfy(value -> value == size / 2);
    }

    @Benchmark
    public int bag_search_hashBag_occurrencesOf() {
        return hashBag.occurrencesOf(distinctValues / 2);
    }

    @Benchmark
    public int bag_search_immutableBag_occurrencesOf() {
        return immutableBag.occurrencesOf(distinctValues / 2);
    }

    @Benchmark
    public boolean bag_search_hashBag_contains() {
